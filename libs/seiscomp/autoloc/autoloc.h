...
*/
	private:
		...
		// Initialize one station at runtime for a pick.
		// Only initializes a station if needed.
	...
	private:
		...
		// Return true if the pick has expired, i.e. is too old
		bool _expired(const Autoloc::DataModel::Pick*) const;

		// Return true if this is a pick that requires amplitudes
		bool _requiresAmplitude(const Autoloc::DataModel::Pick*) const;
...
//		bool _improve(Origin *);
		...
		// TODO: pass it an Arrival instance

		// Try to judge if an origin *may* be a fake
		//
